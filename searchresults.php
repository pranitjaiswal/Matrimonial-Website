...
$leftJoin = "SELECT * FROM users as u left join user_profile as up on u.UserID=up.UserID left join countries as c on u.CountryID=c.CountryID left join states as s on up.StateID= s.StateID";
...
$sqltot = $leftJoin. " where u.Status=1 and u.ApprovalStatus=1 and u.Gender='".$_REQUEST['gender']."' and u.CountryID=".$_REQUEST['countryofresidence']." and (u.Age BETWEEN ".$_REQUEST['agefrom']." and ".$_REQUEST['ageto'].")  ".$photo."   and up.Caste LIKE '%".$caste1."%' order by u.UserID desc";
}
else
{
$sql = $leftJoin. " where u.Status=1 and u.ApprovalStatus=1 and u.Gender='".$_REQUEST['gender']."' and (u.Age BETWEEN ".$_REQUEST['agefrom']." and ".$_REQUEST['ageto'].")  ".$photo."   and up.Caste LIKE '%".$caste1."%' order by u.UserID desc limit $eu, $limit";



$sqltot = $leftJoin. " where u.Status=1 and u.ApprovalStatus=1 and u.Gender='".$_REQUEST['gender']."' and (u.Age BETWEEN ".$_REQUEST['agefrom']." and ".$_REQUEST['ageto'].")  ".$photo."   and up.Caste LIKE '%".$caste1."%' order by u.UserID desc";
...
<html>
<head>
<title>FAMM - Search Results</title>
...
<tr bgcolor="#FB920B" height="18">
 ...
 <?PHP
  ...
  <tr>
    <td style="border-bottom: 1px solid #FEDFCB;"><strong>Name : <?PHP echo stripslashes($row['FName']); echo "&nbsp"; echo $row['LName'];?> </strong></td>
  </tr>
  <tr>
